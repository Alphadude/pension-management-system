import { financeNavLinks } from "@/lib/nav-links";
...
const FinanceDashboardNavigation = () => {
  const pathname = usePathname();

  return (
    <Stack gap={16} className="mt-8 flex-1">
      {financeNavLinks.map((link, index) => (
        <Link
          ...
          className={cn(
            "font-poppins text-grey hover:text-primary flex items-center gap-x-2.5 py-3 pr-[7px] pl-4 text-base leading-[17px]",
            {
              "bg-primary rounded-[8px] text-white shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] hover:text-white":
                link.href === routes.financeDashboard.root
                  ? pathname === routes.financeDashboard.root
...
export default FinanceDashboardNavigation;
